    ...
    'HV-P02-087': [
        {
            ...
            'set': {
                'question_zh': '能夠出場的Guts中的「宮 侑」或「宮 治」，是專指位於舉球區下方Guts中的「宮 侑」，以及位於攻擊區下方Guts中的「宮 治」嗎？',
                'answer_zh': '不，並不以此為限。若Guts所屬的區域沒有特別指定時，不論是發球區、攔網區、接球區、舉球區或攻擊區的任何一個區域，都可以將其中的Guts「宮 侑」與「宮 治」出場。',
            },
        },
        {
            'anchor': {'answer_zh': '不適用於與第二個技能一起出場'},
            'set': {
                'question_zh': '我使用這張卡片的技能，使攻擊區的「宮 治」攻擊值+1。之後，我使用第二張這張卡片的技能，在攻擊區出場另一張「宮 治」，並使該角色的攻擊值+1。此時，攻擊區的「宮 治」其攻擊值會被+2嗎？',
                'answer_zh': '不會，只會有+1。第一張技能所給予的攻擊值+1是針對「當時」作為攻擊角色的「宮 治」；它並不會套用在因第二張技能所出場的另一張「宮 治」上。最終，只會套用第二張技能所給予的攻擊值+1。',
            },
        },
    ],
}
