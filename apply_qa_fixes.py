    ...
    'HV-D03-013': [
        {
            ...
            'set': {'answer_zh': '不，你不能。如果卡片名稱「宮兄弟」更改為「宮侑」或「宮治」，則可以使用。'},
        },
    ],
    # #44 — 「不可以」應為「不需要」
    'HV-P02-024': [
    ...
    'HV-P02-027': [
        {
            ...
            'set': {
                'question_zh': '在對手使用該角色技能後的下一回合中，對手可以對伊達工業的中央攔網手使用HV-P02-090「追分拓朗」的「為自己的伊達工業角色中的一個具有3攔網值的角色添加1攔網值」嗎？',
                'answer_zh': '不能。',
            },
        },
        {
    ...
    'HV-P02-088': [
        {
            ...
            'set': {
                'question_zh': '如果自己的舉球角色是「宮治」並且自己的攻擊角色是「宮侑」，在使用這張Event牌後，對手在下一個回合是否可以將一張Event牌放置在Event區中，來使用HV-P02-003「月島螢」的技能？',
    ...
    'HV-P02-087': [
        {
            ...
            'set': {
                'question_zh': '能夠出場的Guts中的「宮侑」或「宮治」，是專指位於舉球區下方Guts中的「宮侑」，以及位於攻擊區下方Guts中的「宮治」嗎？',
                'answer_zh': '不，並不以此為限。若Guts所屬的區域沒有特別指定時，不論是發球區、攔網區、接球區、舉球區或攻擊區的任何一個區域，都可以將其中的Guts「宮侑」與「宮治」出場。',
            },
        },
        {
            'anchor': {'answer_zh': '不適用於與第二個技能一起出場'},
            'set': {
                'question_zh': '我使用這張卡片的技能，使攻擊區的「宮治」攻擊值+1。之後，我使用第二張這張卡片的技能，在攻擊區出場另一張「宮治」，並使該角色的攻擊值+1。此時，攻擊區的「宮治」其攻擊值會被+2嗎？',
                'answer_zh': '不會，只會有+1。第一張技能所給予的攻擊值+1是針對「當時」作為攻擊角色的「宮治」；它並不會套用在因第二張技能所出場的另一張「宮治」上。最終，只會套用第二張技能所給予的攻擊值+1。',
            },
        },
    ],
}
